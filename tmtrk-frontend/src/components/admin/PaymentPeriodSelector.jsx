...
const PaymentPeriodSelector = ({ onPeriodChange, selectedPeriod, periods = [] }) => {
  ...
  useEffect(() => {
    if (periods.length > 0 && selectedPeriod) {
      ...
      if (index !== -1 && index >= 0 && index < periods.length) {
        setCurrentIndex(index);
      } else if (periods.length > 0) {
        // If selected period not found, default to first available period
        setCurrentIndex(0);
      }
    } else if (periods.length > 0) {
      // If no selected period but we have periods, start with first one
      setCurrentIndex(0);
    }
  ...
  const scrollLeft = () => {
    if (periods.length > 0 && currentIndex > 0) {
  ...
  const scrollRight = () => {
    if (periods.length > 0 && currentIndex < periods.length - 1) {
  ...
  const getVisiblePeriods = () => {
    if (!isScrollable || periods.length === 0) return periods;
    ...
    const end = Math.min(start + 3, periods.length);
    return periods.slice(start, end);
  };

  const getVisibleStartIndex = () => {
    if (!isScrollable || periods.length === 0) return 0;
    ...
    <div className="bg-white rounded-lg shadow-sm border p-4 mb-6">
      ...
      <div className="flex items-center gap-2">
        ...
        {isScrollable && (
          <button
            onClick={scrollLeft}
            disabled={periods.length === 0 || currentIndex <= 0}
        ...
        {isScrollable && (
          <button
            onClick={scrollRight}
            disabled={periods.length === 0 || currentIndex >= periods.length - 1}
            className="p-2 rounded-lg border border-gray-200 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
      ...
      {periods.length === 0 && (
        <div className="text-center py-8 text-gray-500">
          ...
          <div>
            {selectedPeriod && (selectedPeriod.periodNumber <= 0 || selectedPeriod.periodNumber === null || selectedPeriod.periodNumber === undefined)
              ? 'No valid payment periods available'
